...
(function() {
    ...
    function addBulkAction() {
        ...
        if (dropdownMenu.querySelector('.itrblueboost-bulk-category-faq')) {
        ...
        bulkItem.className = 'dropdown-item itrblueboost-bulk-category-faq';
        bulkItem.innerHTML = '<i class="material-icons">auto_awesome</i> ' + itrblueboostBulkCategoryFaqLabel;
        bulkItem.addEventListener('click', handleBulkFaqClick);

        dropdownMenu.appendChild(bulkItem);
        bulkActionAdded = true;

        console.log('ITRBlueBoost: Bulk Category FAQ action added to category list');
    }
    ...
    function createModal() {
        ...
        var modalHtml = `
        <div class="modal fade" id="itrblueboostBulkCategoryFaqModal" tabindex="-1" role="dialog" aria-hidden="true">
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div id="itrblueboost-bulk-category-loading" class="text-center py-4">
                        ...
                        <div id="itrblueboost-bulk-category-form" class="d-none">
                            <div class="alert alert-info">
                                ...
                                <strong id="itrblueboost-bulk-category-count">0</strong> category(ies) selected
                            </div>
                            <div class="form-group">
                                <label for="itrblueboost-bulk-category-prompt">Select a prompt:</label>
                                <select class="form-control" id="itrblueboost-bulk-category-prompt">
                                    <option value="">-- Choose a prompt --</option>
                                </select>
                                <small class="form-text text-muted" id="itrblueboost-bulk-category-prompt-desc"></small>
                            </div>
                        </div>
                        <div id="itrblueboost-bulk-category-progress" class="d-none">
                            <div class="text-center py-3">
                                <div class="spinner-border text-success" role="status">
                                    <span class="sr-only">Generating...</span>
                                </div>
                                <p class="mt-2" id="itrblueboost-bulk-category-progress-text">Generating FAQs... Please wait.</p>
                            </div>
                            <div class="progress" style="height: 25px;">
                                <div class="progress-bar progress-bar-striped progress-bar-animated" role="progressbar"
                                     style="width: 0%;" id="itrblueboost-bulk-category-progress-bar">0%</div>
                            </div>
                        </div>
                        <div id="itrblueboost-bulk-category-result" class="d-none"></div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                        <button type="button" class="btn btn-success" id="itrblueboost-bulk-category-generate-btn" disabled>
                            <i class="material-icons" style="vertical-align: middle;">auto_awesome</i>
                            Generate
                        </button>
                    </div>
    ...
    function handleBulkFaqClick(e) {
        ...
        var countEl = document.getElementById('itrblueboost-bulk-category-count');
        if (countEl) {
            countEl.textContent = selectedIds.length;
        }

        // Reset modal state
        var loadingEl = document.getElementById('itrblueboost-bulk-category-loading');
        var errorEl = document.getElementById('itrblueboost-bulk-category-error');
        var formEl = document.getElementById('itrblueboost-bulk-category-form');
        var progressEl = document.getElementById('itrblueboost-bulk-category-progress');
        var resultEl = document.getElementById('itrblueboost-bulk-category-result');
        var generateBtn = document.getElementById('itrblueboost-bulk-category-generate-btn');
        var promptSelect = document.getElementById('itrblueboost-bulk-category-prompt');
    ...
    function getSelectedCategoryIds() {
        ...
        if (ids.length === 0) {
            ...
            rows.forEach(function(row) {
                var id = row.dataset.categoryId || row.dataset.id || row.getAttribute('data-category-id');
                if (id) {
        ...
        console.log('ITRBlueBoost: Selected category IDs:', ids);
        return ids;
    }

    function loadPrompts() {
        fetch(itrblueboostBulkCategoryFaqPromptsUrl, {
            method: 'GET',
        ...
        .then(function(data) {
            ...
            if (!data.success) {
                document.getElementById('itrblueboost-bulk-category-error').textContent = data.message || 'Error loading prompts.';
                document.getElementById('itrblueboost-bulk-category-error').classList.remove('d-none');
                return;
            }

            var prompts = data.prompts || [];
            if (prompts.length === 0) {
                document.getElementById('itrblueboost-bulk-category-error').textContent = 'No prompts available.';
                document.getElementById('itrblueboost-bulk-category-error').classList.remove('d-none');
                return;
            }

            var select = document.getElementById('itrblueboost-bulk-category-prompt');
        ...
        .catch(function() {
            document.getElementById('itrblueboost-bulk-category-loading').classList.add('d-none');
            document.getElementById('itrblueboost-bulk-category-error').textContent = 'Connection error.';
            document.getElementById('itrblueboost-bulk-category-error').classList.remove('d-none');
        });
    }

    function handleGenerate() {
        var promptId = document.getElementById('itrblueboost-bulk-category-prompt').value;
        ...
        // Show progress
        document.getElementById('itrblueboost-bulk-category-form').classList.add('d-none');
        document.getElementById('itrblueboost-bulk-category-generate-btn').classList.add('d-none');
        document.getElementById('itrblueboost-bulk-category-progress').classList.remove('d-none');

        var progressBar = document.getElementById('itrblueboost-bulk-category-progress-bar');
        ...
        .then(function(data) {
            ...
            document.getElementById('itrblueboost-bulk-category-progress').classList.add('d-none');

            var resultDiv = document.getElementById('itrblueboost-bulk-category-result');
        ...
        .catch(function() {
            document.getElementById('itrblueboost-bulk-category-progress').classList.add('d-none');
            document.getElementById('itrblueboost-bulk-category-result').innerHTML =
                '<div class="alert alert-danger">Connection error.</div>';
            document.getElementById('itrblueboost-bulk-category-result').classList.remove('d-none');
        });
    }
